...
Actions:
- search_members: Search inbox/connections. Params: q (fuzzy name search — a single distinctive term like a last name works best, avoids false matches from common first names), type (inbox|unread_inbox|connections), pipeline_name, step_title, linkedin_public_ids (array of public profile slugs for exact lookup, e.g. ["janedoe"]), updated_since, limit, offset, include_history (opt-in; default false; adds position history — past_positions, years_of_experience, and position-date fields — at a token cost, so request only when reconstructing a contact's job changes).
